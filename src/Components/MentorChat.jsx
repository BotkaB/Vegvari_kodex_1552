  ...
  return (
    <section className="flex flex-col rounded-3xl border border-[#3a3f4d] bg-[#1f222b]/90 p-4 shadow-xl">
      ...
      <div className="mb-3 flex items-center justify-between rounded-2xl bg-[#14161b]/70 p-2 border border-[#3a3f4d]">
        <span className="text-xs font-medium text-stone-400 px-2">Megjelenítés:</span>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-2">
          ...
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-2xl p-3 text-sm shadow ${msg.role === 'user' ? 'bg-cyan-950/40 border border-cyan-600/40 text-cyan-50' : 'bg-[#14161b] border border-[#3a3f4d] text-stone-200'}`}>
              {msg.role === 'user' ? (
                ...
                <p className="text-red-400">Hiba: {msg.content}</p>
              ) : (
        ...
        {chatSubmitting && <div className="text-xs text-stone-400 animate-pulse p-2">A 4 mentor válaszol...</div>}
      </div>
