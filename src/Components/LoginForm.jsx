          ...
          <form onSubmit={handleSubmit} className="space-y-4" aria-label="Bejelentkezési űrlap">
            <div>
              ...
              <input
                ...
                className="w-full rounded-2xl border border-[#3a3f4d] bg-[#14161b] px-4 py-2 text-white outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-400 shadow-inner"
            ...
            {error && (
              <div className="rounded-2xl border border-red-500/50 bg-red-500/10 px-4 py-2 text-sm text-red-200" role="alert">
                {error}
              </div>
            )}
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 px-4 py-2.5 font-bold text-stone-950 transition hover:brightness-110 disabled:opacity-50 shadow-lg focus:outline-none focus:ring-2 focus:ring-amber-400"
            >
